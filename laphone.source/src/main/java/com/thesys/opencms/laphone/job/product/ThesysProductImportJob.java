...
public class ThesysProductImportJob extends ThesysAbstractJob{
	...
	public void doImport(){
		try{
			...
			String inboxFolder = client.getInboxFolder(ThesysSapFtpClient.FILE_TYPE_PRODUCT_MAIN);
			String backupFolder = client.getBackupFolder(ThesysSapFtpClient.FILE_TYPE_PRODUCT_MAIN);
			String errorFolder = client.getErrorFolder(ThesysSapFtpClient.FILE_TYPE_PRODUCT_MAIN);
			...
			if(fileNames!=null && fileNames.length>0){
				for(int i=0;i<fileNames.length;i++){
					...
					if(in!=null){
						...
						try{
							importHandler.importProductCsv(in);
