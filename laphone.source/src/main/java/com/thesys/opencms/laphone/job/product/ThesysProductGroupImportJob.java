...
public class ThesysProductGroupImportJob extends ThesysAbstractJob{

	protected static final Log LOG = CmsLog.getLog(ThesysProductGroupImportJob.class);
	...
	public void doImport(){
		try{
			...
			String inboxFolder = client.getInboxFolder(ThesysSapFtpClient.FILE_TYPE_PRODUCT_GROUP);
			String backupFolder = client.getBackupFolder(ThesysSapFtpClient.FILE_TYPE_PRODUCT_GROUP);
			String errorFolder = client.getErrorFolder(ThesysSapFtpClient.FILE_TYPE_PRODUCT_GROUP);			
			...
			if(fileNames!=null && fileNames.length>0){
				for(int i=0;i<fileNames.length;i++){
					...
					if(in!=null){
						...
						try{
							importHandler.importGroupCsv(in);
