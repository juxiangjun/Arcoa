...
import com.thesys.opencms.laphone.imp.ThesysCvsReturnImportHandler;
import com.thesys.opencms.laphone.job.ThesysAbstractJob;
import com.thesys.opencms.laphone.job.ftp.ThesysCvsFtpClient;

public class ThesysCvsReturnImportJob extends ThesysAbstractJob{

	protected static final Log LOG = CmsLog.getLog(ThesysCvsReturnImportJob.class);
	...
	public void doImport(){
		try{
			...
			//過濾本日之所有檔案
			String folder = client.getFolder(ThesysCvsFtpClient.FILE_TYPE_F07);
			String fileFilter = client.getFileName(ThesysCvsFtpClient.FILE_TYPE_F07);
			//取得ftp資料夾清單
			...
			if(fileNames!=null && fileNames.length>0){
				for(int i=0;i<fileNames.length;i++){
					...
					if(in!=null){
						//Step 2: 匯入組案價格檔
						ThesysCvsReturnImportHandler importHandler = new ThesysCvsReturnImportHandler(getCmsObject());	
						try{
