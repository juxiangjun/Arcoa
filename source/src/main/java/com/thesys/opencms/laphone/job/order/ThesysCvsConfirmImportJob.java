...
import com.thesys.opencms.laphone.imp.ThesysCvsConfirmImportHandler;
import com.thesys.opencms.laphone.job.ThesysAbstractJob;
import com.thesys.opencms.laphone.job.ftp.ThesysCvsFtpClient;

public class ThesysCvsConfirmImportJob extends ThesysAbstractJob{

	protected static final Log LOG = CmsLog.getLog(ThesysCvsConfirmImportJob.class);
	...
	public void doImport(){
		try{
			...
			//過濾本日之所有檔案
			String folder = client.getFolder(ThesysCvsFtpClient.FILE_TYPE_F11);
			String fileFilter = client.getFileName(ThesysCvsFtpClient.FILE_TYPE_F11);
			//取得ftp資料夾清單
			...
			if(fileNames!=null && fileNames.length>0){
				for(int i=0;i<fileNames.length;i++){
					...
					if(in!=null){
						//Step 2: 匯入組案價格檔
						ThesysCvsConfirmImportHandler importHandler = new ThesysCvsConfirmImportHandler(getCmsObject());	
						try{
