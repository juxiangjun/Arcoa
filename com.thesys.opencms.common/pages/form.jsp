...
<%
		...
		for (int i = pagingPos, n = fields.size(); i < n; i++) {
			...
			String html = field.buildHtml(cms, messages, errorMessage, formConfiguration.isShowMandatory(), infoMessage);
			
			
			//若為驗證碼
			if("captchaphrase".equals(field.getName())){
				// 取得驗證碼的位置
				String fieldStart = messages.key("form.html.field.start");
				String fieldEnd = messages.key("form.html.field.end");
				String captchaPart1 = html.substring(0,html.indexOf(fieldStart)+fieldStart.length());
				String captchaPart2 =  html.substring(html.indexOf(fieldStart)+fieldStart.length(),html.indexOf(fieldEnd ));
				String captchaPart3 = html.substring(html.indexOf(fieldEnd ));
				String captchaImage = captchaPart2.substring(0,captchaPart2.indexOf("<br/>"));
				String captchaField = captchaPart2.substring(captchaPart2.indexOf("<br/>")+5,captchaPart2.length());
		
				//印出驗證碼欄位
				String captchaInputHtml  = captchaPart1 +messages.key("form.html.captcha.input.start") + captchaField+ messages.key("form.html.captcha.input.end") +captchaPart3 ;
				
				//變更錯誤訊息
				if(field.getValue().length()==0){
					captchaInputHtml = captchaInputHtml.replaceFirst(messages.key("form.error.validation"),messages.key("form.error.mandatory"));
				}else{
					captchaInputHtml = captchaInputHtml.replaceFirst(messages.key("form.error.validation"),messages.key("form.error.captcha"));
				}
				//印出驗證碼圖片
				String captchaImgHtml = messages.key("form.html.row.start")+messages.key("form.html.label.start")+"&nbsp;"+messages.key("form.html.label.end")+
					      		messages.key("form.html.field.start")+
					      		messages.key("form.html.captcha.image.start")+
					      		captchaImage+messages.key("form.html.captcha.image.text")+
					      		messages.key("form.html.captcha.image.end")+
					      		messages.key("form.html.field.end")+messages.key("form.html.row.end");
				html = captchaInputHtml + captchaImgHtml; 		
			
			}
			
			//若有訊息，改在同一行 add by Maggie
			boolean after = Boolean.parseBoolean(messages.key("form.html.text.after"));
			if(after){
				html =  html.replaceAll(messages.key("form.html.field.end")+"\n"+
				       	messages.key("form.html.row.end")+"\n"+
					messages.key("form.html.row.start")+"\n"+
					messages.key("form.html.label.start")+messages.key("form.html.label.end")+"\n"+
					messages.key("form.html.text.start"),"");
				
				int errorIndex = html.indexOf(messages.key("form.html.error.start"));
				if(errorIndex>-1){
					//取得錯誤訊息內容
					String errorMsg = html.substring(errorIndex,html.indexOf(messages.key("form.html.error.end"),errorIndex))+messages.key("form.html.error.end");
					//清除錯誤內容
					html = html.replaceAll(errorMsg,"");
					//加入錯誤內容
					html = html.replaceAll(messages.key("form.html.field.end")+"\n"+messages.key("form.html.row.end"),errorMsg+messages.key("form.html.field.end")+"\n"+messages.key("form.html.row.end"));
				}
...
<input type="submit" value="<%=messages.key("form.button.submit")%>" <%=messages.key("form.button.submit.attributes")%> />  <%  
			if (formConfiguration.isShowReset()) {
%>&nbsp;<input type="reset"
	value="<%=messages.key("form.button.reset")%>"
	<%=messages.key("form.button.reset.attributes")%> />
<%
			}
...
<%=messages.key("form.html.button.start")%><input type="submit"
	...
	<%=messages.key("form.button.downloaddata.attributes")%> /><%=messages.key("form.html.button.end")%>
<%=messages.key("form.html.row.end")%>
<%
		}
